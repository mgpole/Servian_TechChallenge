...
resource "azurerm_monitor_autoscale_setting" "app_service_autoscale" {
  name                = join("-", [var.ENVIRONMENT_NAME, var.PRODUCT_NAME, "web", "autoscale"])
  resource_group_name = azurerm_resource_group.pri_rg.name
  location            = var.pri_location
  target_resource_id  = azurerm_app_service_plan.web_asp.id
  profile {
    name = "default"
    capacity {
      default = 1
    ...
    rule {
      metric_trigger {
        ...
        metric_resource_id = azurerm_app_service_plan.web_asp.id
        time_grain         = "PT1M"
        statistic          = "Average"
        time_window        = "PT5M"
